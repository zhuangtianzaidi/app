        ...
        <div class="details clearfix">
          <div class="sui-navbar">
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{ active: isOne }" @click="asd('1')">
                  <a href="#"
                    >综合<span
                      v-show="isOne"
                      class="iconfont"
                ...
                <li :class="{ active: isTwo }" @click="asd('2')">
                  <a href="#"
                    >价格<span
                      v-show="isTwo"
                      class="iconfont"
...
export default {
  ...
  mounted() {
    ...
    this.getgo();
    this.$bus.$on("pinpai", (dsata) => {
      this.searchParams.trademark = `${dsata.tmId}:${dsata.tmName}`;
      this.getgo();
    });
    this.$bus.$on("shuxing", (attr, attrValue) => {
  ...
  methods: {
    ...
    removeCategoryName() {
      ...
      this.getgo();
      //地址栏也要发生变化，自己跳自己 但要严谨一些，有些带有搜索值params要带过去
    ...
    asd(flag) {
      ...
      let pan = originOrder.split(":")[0];
      let panduan = originOrder.split(":")[1];
      let newhe = "";
      if (flag == pan) {
        newhe = `${pan}:${panduan == "desc" ? "asc" : "desc"}`;
      } else {
        newhe = `${flag}:${"desc"}`;
      }
      this.searchParams.order = newhe;
      this.getgo();
    },
    huidiao(newdata) {
      this.searchParams.pageNo = newdata;
      this.getgo();
    },
  },
  computed: {
    ...mapGetters(["goodsList", "total"]),
    /* 不使用getters计算属性写法 ...mapState({
